(function () {
  ...
  /* ── UTILS ── */
  window.syncPicker = function (p, h) {
    document.getElementById(h).value = document.getElementById(p).value;
  };
  window.syncHex = function (h, p) {
    var v = document.getElementById(h).value;
    if (/^#[0-9a-fA-F]{6}$/.test(v)) document.getElementById(p).value = v;
  };
  function val(id) {
  ...
  window.insertVar = function (inputId, text) {
    ...
    inp.value =
      inp.value.slice(0, s) + text + inp.value.slice(inp.selectionEnd);
    updatePreview();
  };
  ...
  window.toggleOpt = function (checkId, fieldsId, blockId) {
    ...
    if (b) b.classList.toggle("enabled", on);
  };

  window.setView = function (mode) {
    document
      .getElementById("btnDesktop")
    ...
    document
      ...
      .classList.toggle("mobile", mode === "mobile");
  ...
  window.saveSelFor = function (edId) {
    ...
    if (sel && sel.rangeCount > 0)
      savedSelMap[edId] = sel.getRangeAt(0).cloneRange();
  };
  function restoreSel(edId) {
    var r = savedSelMap[edId];
    if (!r) return false;
    var ed = document.getElementById(edId);
    ed.focus();
    var sel = window.getSelection();
  ...
  window.onGlobalColorInput = function () {
    document.getElementById("globalColorHex").value =
      document.getElementById("globalColorPicker").value;
  };
  window.onGlobalHexInput = function () {
    var h = document.getElementById("globalColorHex").value;
    if (/^#[0-9a-fA-F]{6}$/.test(h))
      document.getElementById("globalColorPicker").value = h;
  };

  window.applyGlobalColor = function () {
    var color = document.getElementById("globalColorHex").value;
    if (!color || !currentEditorId) {
    ...
    if (restoreSel(currentEditorId)) {
      ...
      var wrap = document
        .getElementById(currentEditorId)
        .closest(".block-wrap");
      if (wrap) {
        var dot = wrap.querySelector(".color-trigger .color-dot");
  ...
  document.addEventListener("mousedown", function (e) {
    ...
    if (!popup.contains(e.target) && !e.target.closest(".color-trigger"))
      popup.classList.remove("open");
  });

  /* ── FONT SIZE ── */
  window.applyFontSize = function (selectEl, edId) {
    var size = selectEl.value;
    if (!size) return;
    var r = savedSelMap[edId];
    if (!r || r.collapsed) {
      selectEl.value = "";
  ...
  /* ── RICH TEXT ── */
  window.fmtCmd = function (e, edId, cmd) {
    e.preventDefault();
    document.getElementById(edId).focus();
    document.execCommand(cmd, false, null);
    updatePreview();
  };
  window.insertLinkFor = function (e, edId) {
    e.preventDefault();
  ...
  function buildToolbar(edId) {
    return (
      '<button class="tb-btn" onmousedown="fmtCmd(event,\'' +
      edId +
      "','bold')\"><b>B</b></button>" +
      '<button class="tb-btn" onmousedown="fmtCmd(event,\'' +
      edId +
      ...
      '<button class="tb-btn" onmousedown="fmtCmd(event,\'' +
      edId +
      "','underline')\"><u>U</u></button>" +
      '<div class="tb-sep"></div>' +
      '<select class="tb-size" onmousedown="saveSelFor(\'' +
      edId +
      "')\" onchange=\"applyFontSize(this,'" +
      edId +
      "')\">" +
      ...
      "</select>" +
      '<div class="tb-sep"></div>' +
      '<button class="color-trigger" onmousedown="saveSelFor(\'' +
      edId +
      "')\" onclick=\"openColorPopup(this,'" +
      edId +
      "')\">" +
      ...
      '<button class="tb-btn" onmousedown="fmtCmd(event,\'' +
      edId +
      "','insertUnorderedList')\">• Lista</button>" +
      '<button class="tb-btn" onmousedown="fmtCmd(event,\'' +
      edId +
      "','insertOrderedList')\">1. Lista</button>" +
      '<div class="tb-sep"></div>' +
      '<button class="tb-btn" onmousedown="insertLinkFor(event,\'' +
      edId +
      "')\">Link</button>"
    );
  }
  ...
  window.addBlock = function () {
    ...
    div.innerHTML =
      ...
      '<span class="block-label">Bloco ' +
      blockCount +
      "</span>" +
      '<div class="block-header-right">' +
      '<button class="seg-btn active" onclick="setAlign(\'' +
      id +
      "','left')\">Esq</button>" +
      '<button class="seg-btn" onclick="setAlign(\'' +
      id +
      "','center')\">Centro</button>" +
      '<button class="seg-btn" onclick="setAlign(\'' +
      id +
      "','right')\">Dir</button>" +
      '<div class="tb-sep" style="height:16px;margin:0 3px;"></div>' +
      '<button class="block-remove" onclick="removeBlock(\'' +
      id +
    ...
    edVis.addEventListener("input", function () {
      updatePreview();
    });
  ...
  window.setAlign = function (blockId, align) {
    ...
    block.querySelectorAll(".seg-btn").forEach(function (b) {
      var t = b.textContent.trim();
      if (map[t] !== undefined) b.classList.toggle("active", map[t] === align);
    });
    updatePreview();
  };
  ...
  function generateEmailHTML() {
    ...
    var logoOn = checked("logoEnabled"),
      logoUrl = val("logoUrl"),
      logoLink = val("logoLink");
    var banner = val("bannerUrl"),
      bannerLnk = val("bannerLink");
    var titleOn = checked("titleEnabled"),
      titleTxt = val("titleText"),
      titleClr = val("titleColorHex") || "#242424";
    var btnOn = checked("btnEnabled"),
      btnTxt = val("btnText"),
      btnLnk = val("btnLink");
    var btnBg = val("btnBgHex") || "#2563eb",
      btnTxtClr = val("btnTxtHex") || "#242424",
      btnBorder = val("btnBorderHex") || "#000000";
    var btnTab = checked("btnNewTab");
    var bgExt = val("bgHex") || "#f0f0f0",
      cardVerde = val("cardVerdeHex") || "#1e293b";
    var cardBg = val("cardBgHex") || "#ffffff",
      cardBorder = val("cardBorderHex") || "#e1e1e6";
    ...
    var instaOn = checked("instaOn"),
      instaUrl = val("instaUrl");
    var teleOn = checked("teleOn"),
      teleUrl = val("teleUrl");
    var ytOn = checked("ytOn"),
      ytUrl = val("ytUrl");
    var waOn = checked("waOn"),
      waUrl = val("waUrl");
    var iconStyle = val("socialIconStyle") || "color";
    var iconColor = (val("socialIconColorHex") || "#ffffff").replace("#", "");
    var iconScale = val("socialIconSize") === "sm" ? 0.7 : 1;
    ...
    if (logoOn && logoUrl) {
      var lImg =
        '<img alt="Logo" src="' +
        logoUrl +
        '" style="display:inline-block;outline:none;border:none;text-decoration:none;max-width:100%;" width="250" />';
      var lInner = logoLink
        ? '<a href="' +
          logoLink +
        ...
        : lImg;
      logoCard =
        '<table align="center" width="100%" border="0" cellpadding="0" cellspacing="0" role="presentation" style="max-width:600px;margin:0 auto 10px;background-color:' +
        cardVerde +
        ';color:#f0f0f0;border-radius:15px;overflow:hidden;"><tbody><tr style="width:100%"><td><h1 style="padding:10px 0 0;text-align:center;">' +
        lInner +
    ...
    if (banner) {
      var bImg =
        '<img alt="Banner" src="' +
        banner +
        '" style="display:block;outline:none;border:none;text-decoration:none;width:100%;height:auto;" width="600" />';
      var bInner = bannerLnk
        ? '<a href="' +
          bannerLnk +
        ...
        : bImg;
      bannerCard =
        '<table align="center" width="100%" border="0" cellpadding="0" cellspacing="0" role="presentation" style="max-width:600px;margin:0 auto 10px;background-color:#eeeeee;color:#242424;border-radius:15px;border:1px solid ' +
        cardVerde +
        ';overflow:hidden;"><tbody><tr style="width:100%"><td><table align="center" width="100%" border="0" cellpadding="0" cellspacing="0" role="presentation" style="padding:0;text-align:center;"><tbody><tr><td>' +
        bInner +
    ...
    var titleHtml =
      titleOn && titleTxt
        ? '<p style="font-size:24px;line-height:150%;font-weight:900;margin-bottom:15px;text-align:center;margin-top:16px;color:' +
          titleClr +
          ';">' +
          esc(titleTxt) +
          "</p>"
    ...
    document.querySelectorAll(".block-wrap").forEach(function (block) {
      ...
      if (!content) return;
      blocksHtml +=
        '<p style="font-size:16px;line-height:165%;font-weight:500;margin-top:16px;margin-bottom:16px;text-align:' +
    ...
    if (btnOn && btnTxt) {
      var bHref = btnLnk || "#";
      var bTarget = btnTab && btnLnk ? ' target="_blank"' : "";
      var borderOn = checked("btnBorderOn");
      var borderCss = borderOn ? "border:2px solid " + btnBorder + ";" : "border:none;";
      btnHtml =
        '<div style="margin-top:30px;text-align:center;">' +
        "<!--[if mso]>" +
        '<v:roundrect xmlns:v="urn:schemas-microsoft-com:vml" xmlns:w="urn:schemas-microsoft-com:office:word"' +
        ' href="' + bHref + '"' +
        ' style="height:46px;v-text-anchor:middle;width:200px;" arcsize="15%"' +
        (borderOn ? ' strokecolor="' + btnBorder + '"' : ' stroked="f"') +
        ' fillcolor="' + btnBg + '">' +
        "<w:anchorlock/>" +
        '<center style="color:' + btnTxtClr + ';font-family:Arial,sans-serif;font-size:16px;font-weight:900;text-transform:uppercase;">' +
        esc(btnTxt) +
        "</center></v:roundrect>" +
        "<![endif]-->" +
        "<!--[if !mso]><!-->" +
        '<a href="' + bHref + '"' + bTarget +
        ' style="line-height:100%;text-decoration:none;display:inline-block;max-width:100%;background-color:' + btnBg +
        ";color:" + btnTxtClr +
        ";font-weight:900;padding:12px 30px;font-size:16px;border-radius:10px;" + borderCss +
        'text-transform:uppercase;font-family:Arial,sans-serif;"><span style="max-width:100%;display:inline-block;line-height:120%;">' +
        esc(btnTxt) +
        "</span></a>" +
        "<!--<![endif]-->" +
        "</div>";
    ...
    if (footerOn) {
      var socCells = "";
      // ícones: coloridos (flaticon) ou monocromáticos com cor custom (icons8)
      var icons = {
        insta: {
          color: "https://cdn-icons-png.flaticon.com/512/2111/2111463.png",
          mono: "https://img.icons8.com/ios-filled/70/" + iconColor + "/instagram-new.png",
          alt: "Instagram", radius: "100%", w: 35
        },
        tele: {
          color: "https://cdn-icons-png.flaticon.com/512/2111/2111644.png",
          mono: "https://img.icons8.com/ios-filled/70/" + iconColor + "/telegram-app.png",
          alt: "Telegram", radius: "10%", w: 32
        },
        yt: {
          color: "https://cdn-icons-png.flaticon.com/512/1384/1384060.png",
          mono: "https://img.icons8.com/ios-filled/70/" + iconColor + "/youtube-play.png",
          alt: "YouTube", radius: "10%", w: 35
        },
        wa: {
          color: "https://cdn-icons-png.flaticon.com/512/733/733585.png",
          mono: "https://img.icons8.com/ios-filled/70/" + iconColor + "/whatsapp.png",
          alt: "WhatsApp", radius: "10%", w: 34
        }
      };
      function socCell(key, on, url) {
        if (!on) return "";
        var ic = icons[key];
        var src = iconStyle === "mono" ? ic.mono : ic.color;
        var w = Math.round(ic.w * iconScale);
        var pad = iconScale < 1 ? "0 10px" : "0 15px";
        return (
          '<td style="padding:' + pad + ';"><a href="' + (url || "#") +
          '" target="_blank" rel="noopener noreferrer"><img alt="' + ic.alt +
          '" src="' + src +
          '" style="display:block;outline:none;border:none;text-decoration:none;border-radius:' + ic.radius +
          ';" width="' + w + '" /></a></td>'
        );
      }
      socCells += socCell("insta", instaOn, instaUrl);
      socCells += socCell("tele", teleOn, teleUrl);
      socCells += socCell("yt", ytOn, ytUrl);
      socCells += socCell("wa", waOn, waUrl);

      footerCard =
        '<table align="center" width="100%" border="0" cellpadding="0" cellspacing="0" role="presentation" style="max-width:600px;margin:0 auto 10px;background-color:' +
        cardVerde +
        ';color:#f0f0f0;border-radius:15px;overflow:hidden;"><tbody><tr style="width:100%"><td>' +
        '<p style="font-size:14px;line-height:24px;color:' +
        fLegalColor +
        ';margin-top:20px;margin-bottom:10px;text-align:center;padding-inline:40px;">' +
        fLegal +
        "</p>" +
        (socCells
          ? '<table align="center" width="100%" border="0" cellpadding="0" cellspacing="0" role="presentation" style="text-align:center;"><tbody><tr><td>' +
            '<p style="font-size:15px;line-height:24px;color:' +
            fSocColor +
            ';margin-bottom:16px;padding-inline:40px;margin-top:16px;">' +
            esc(fSocText) +
            "</p>" +
            '<table style="margin:0 auto" cellpadding="0" cellspacing="0" role="presentation"><tr>' +
            socCells +
            "</tr></table>" +
            "</td></tr></tbody></table>"
          : "") +
        ...
        '<p style="font-size:12px;line-height:24px;color:' +
        fUnsubColor +
        ...
        '" style="color:' +
        fUnsubColor +
        ';text-decoration:underline;">clique aqui</a>.</p>' +
        "</td></tr></tbody></table>" +
        "</td></tr></tbody></table>";
    }
    ...
    return (
      ...
      '<html xmlns:v="urn:schemas-microsoft-com:vml" xmlns:o="urn:schemas-microsoft-com:office:office" style="margin:0;padding:0" dir="ltr" lang="pt-BR">' +
      ...
      '<td style="margin:0;padding:0;padding-top:50px;padding-bottom:50px;background-color:' +
      bgExt +
      ...
      '<table align="center" width="100%" border="0" cellpadding="0" cellspacing="0" role="presentation" style="max-width:600px;margin:0 auto 10px;background-color:' +
      cardBg +
      ";color:#242424;border-radius:15px;border:1px solid " +
      cardBorder +
      ';overflow:hidden;">' +
  ...
  window.copyHTML = function () {
    navigator.clipboard.writeText(generateEmailHTML()).then(function () {
      showToast("Código HTML copiado — cole no seu ESP");
    }).catch(function () {
      showToast("Não foi possível copiar. Tente novamente.", true);
    });
  };
  ...
  window.downloadHTML = function () {
    ...
    showToast("Arquivo email.html baixado");
  };
  ...
  window.copyForGmail = function () {
    ...
    try {
      ...
      navigator.clipboard.write([item]).then(function () {
        showToast("Copiado! Abra o Gmail e cole no corpo da mensagem");
      }).catch(function () {
        showToast("Navegador não suportou. Use \u201CBaixar\u201D, abra o arquivo e copie de lá.", true);
      });
    } catch (e) {
      showToast("Navegador não suportou. Use \u201CBaixar\u201D, abra o arquivo e copie de lá.", true);
    }
  ...
  /* ── ESTILO DOS ÍCONES ── */
  window.toggleIconColor = function () {
    var mono = document.getElementById("socialIconStyle").value === "mono";
    document.getElementById("iconColorField").style.display = mono ? "" : "none";
  };
  ...
  function applyDefaults() {
    ...
    document.getElementById("footerUnsub").value =
      "Para cancelar o recebimento destes e-mails,";
    ...
    document.getElementById("socialIconStyle").value = "color";
    document.getElementById("socialIconSize").value = "md";
    document.getElementById("socialIconColorHex").value = "#ffffff";
    document.getElementById("socialIconColor").value = "#ffffff";
    document.getElementById("btnBorderOn").checked = true;
    toggleIconColor();
    [
      "logoEnabled",
      "titleEnabled",
  ...
  window.confirmNovoEmail = function () {
    closeConfirm();
    try {
      localStorage.removeItem(SAVE_KEY);
    } catch (e) {}
    resetToDefaults();
    showToast("Novo e-mail — tudo voltou ao padrão");
  };
  ...
  var FIELDS = [
    ...
    "instaUrl", "teleUrl", "ytUrl", "waUrl", "socialIconStyle", "socialIconColorHex", "socialIconSize",
  ];
  var CHECKS = [
    "logoEnabled", "titleEnabled", "btnEnabled", "btnNewTab",
    "footerEnabled", "instaOn", "teleOn", "ytOn", "waOn", "btnBorderOn",
  ];
  ...
  function restoreState() {
    ...
    [
      ...
      "footerSocialColorHex", "footerUnsubColorHex", "socialIconColorHex",
